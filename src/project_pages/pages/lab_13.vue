<script setup>
import ProjectPageTemplate from "@/components/project/ProjectPageTemplate.vue";
import YoutubeEmbed from "@/components/generic/YoutubeEmbed.vue";

let props = defineProps({
  test: {
    type: String,
    default: null,
  },
});

let proj_name = 'lab_13'

</script>

<template>
  <project-page-template :project_name="proj_name" :image_loader="false">

    <template #content>

      <div class="vimeo_video">
        <youtube-embed id="xvvaOdXaeJY" :muted="1"/>
        <h5 style="text-align: center;padding-top: 10px">Final product, not representative of current skill. Keeping it
          around for archival purposes</h5>
      </div>

      <div class="vimeo_video">
        <youtube-embed id="URB7SUfJUgw" :muted="0"/>
        <h5 style="text-align: center;padding-top: 10px">Behind the scenes</h5>
      </div>

    </template>

  </project-page-template>
</template>

<style scoped>
.single_image {
  /*outline: 1px solid red;*/
  display: flex;
  flex-flow: column nowrap;
  align-items: center;
  gap: 10px;
}

.vimeo_video {
  justify-content: center;
  width: 100%;
}

.grid_image {
  width: 100%;
  height: 100%;
  max-height: 80vh;
  object-fit: cover
}
</style>